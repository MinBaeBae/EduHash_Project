...
const Todolist = () => {
  const { completionPercent, dispatch, lastAccessDate } = useContext(Context);
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("lastAccessDate") != null) {


      if (
        localStorage.getItem("lastAccessDate") != lastAccessDate
      ) {
        let array = JSON.parse(localStorage.getItem("toDoList"));
        array.forEach(i => {
          i.list.forEach(j => {
            j.time= "", // 운동하는데 걸린 시간
            j.isDone= false, // 운동 완료 유무
            j.inProgress= false // 현재 진행중인 운동
          });
        });

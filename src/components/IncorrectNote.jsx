...
*/
  ...
  const [lastQuiz, setLastQuiz] = useState(-1);
...
*/
  ...
  const url = "/notedata/numList.json";
    ...
    if (count < quizList.length - 1) {
      ...
      try {
        const res = await axios
          .get(
            `/notedata/odab${randomExamNumList[i]}.json`
          )
          .then(res => {
...
const NextBtn = styled.button`
  ...
  background-color: white;
  display: ${props => {
    return props.count == props.lastQuiz ? "none" : "block";
  }};
`;
